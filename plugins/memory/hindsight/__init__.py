...
from .routing import (
    RecallPlan as _RecallPlan,
    arecall_with_fail_open as _arecall_with_fail_open,
    filter_results as _filter_routing_results,
    normalize_routes as _normalize_recall_routes,
    normalize_tags as _normalize_recall_tags,
    parse_bool as _parse_recall_bool,
    valid_match as _valid_recall_match,
)
...
class _RecallResult:
    """Recall text/count pair that remains tuple-unpackable for pre-refactor callers."""
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def _apply_recall_settings(self, cfg: dict) -> None:
        ...
        self._recall_tags_match = _valid_recall_match(cfg.get("recall_tags_match"), default="any")
        self._recall_auto_route = _parse_recall_bool(cfg.get("recall_auto_route", cfg.get("recall_domain_routing")), False)
        self._recall_domain_routing = self._recall_auto_route
        self._recall_auto_route_fail_open = _parse_recall_bool(cfg.get("recall_auto_route_fail_open"), False)
        self._recall_routes = _normalize_recall_routes(cfg.get("recall_routes"))
        with contextlib.suppress(TypeError, ValueError):
            self._recall_max_results = max(0, int(cfg.get("recall_max_results", 0) or 0))
    ...
    def _select_recall_route(self, query: str) -> dict[str, Any] | None:
        if not self._recall_auto_route or not self._recall_routes:
            return None
        chat_id = str(getattr(self, "_chat_id", "") or "").strip()
        chat_name = str(getattr(self, "_chat_name", "") or "").strip().casefold()
        query_text = str(query or "").casefold()
        for route in self._recall_routes.values():
            if chat_id and chat_id in _normalize_recall_tags(route.get("chat_ids")):
                return route
        for route in self._recall_routes.values():
            names = {value.casefold() for value in _normalize_recall_tags(route.get("chat_names"))}
            if chat_name and chat_name in names:
                return route
        for route in self._recall_routes.values():
            if any(value.casefold() in query_text or value.casefold() in chat_name
                   for value in _normalize_recall_tags(route.get("keywords"))):
                return route
        for route in self._recall_routes.values():
    ...
    @staticmethod
    def _route_tag_kwargs(kwargs: dict[str, Any], tags: list[str], match: str, exclude: list[str]) -> None:
        kwargs.pop("tags", None)
        kwargs.pop("tags_match", None)
        kwargs.pop("tag_groups", None)
        kwargs["_hindsight_positive_tags"] = list(tags)
        kwargs["_hindsight_tags_match"] = match
        kwargs["_hindsight_exclude_tags"] = list(exclude)
        if tags and exclude:
            kwargs["tag_groups"] = [{"and": [{"tags": tags, "match": match}, {"not": {"tags": exclude, "match": "any_strict"}}]}]
        elif tags:
            kwargs["tags"] = tags
            kwargs["tags_match"] = match
        elif exclude:
            kwargs["tag_groups"] = [{"not": {"tags": exclude, "match": "any_strict"}}]

    def _build_recall_plan(self, query: str) -> _RecallPlan:
        route = None if self._recall_tags_explicit else self._select_recall_route(query)
        active_tags = list(self._recall_tags or []) if self._recall_tags_explicit else _normalize_recall_tags((route or {}).get("tags"))
        exclude_tags = _normalize_recall_tags((route or {}).get("exclude_tags"))
        request_query = str(query or "")
        ...
        kwargs: dict[str, Any] = {
            "bank_id": self._bank_id, "query": request_query,
            "budget": self._budget, "max_tokens": self._recall_max_tokens,
        }
        route_types = _normalize_recall_tags((route or {}).get("types"))
        types = self._recall_types if self._recall_types_explicit or not route_types else route_types
        if types:
            kwargs["types"] = list(types)
        match = self._recall_tags_match if self._recall_tags_explicit else _valid_recall_match((route or {}).get("tags_match"))
        self._route_tag_kwargs(kwargs, active_tags, match, exclude_tags)
        auto_routed = bool(route is not None and not self._recall_tags_explicit and active_tags)
        fallback_kwargs: list[dict[str, Any]] = []
        fallback_sets = (route or {}).get("fallback_tags")
        if isinstance(fallback_sets, str):
            fallback_sets = [fallback_sets]
        if auto_routed and not fallback_sets and len(active_tags) > 1:
            fallback_sets = [active_tags[:1]]
        if auto_routed and fallback_sets:
            for tags in fallback_sets if isinstance(fallback_sets, list) else []:
                if isinstance(tags, str):
                    tags = [tags]
                fallback = dict(kwargs)
                self._route_tag_kwargs(fallback, _normalize_recall_tags(tags), _valid_recall_match((route or {}).get("fallback_tags_match")), exclude_tags)
                fallback_kwargs.append(fallback)
        unfiltered = dict(kwargs)
        if auto_routed:
            self._route_tag_kwargs(unfiltered, [], "all_strict", exclude_tags)
        cap = (route or {}).get("max_results", self._recall_max_results) if route else self._recall_max_results
        with contextlib.suppress(TypeError, ValueError):
            cap = max(0, int(cap or 0))
        return _RecallPlan(kwargs, route, auto_routed, auto_routed and self._recall_auto_route_fail_open,
                           tuple(fallback_kwargs), unfiltered if auto_routed else None,
                           tuple(exclude_tags), int(cap or 0))
    ...
    def _recall(self, query: str) -> list:
        ...
        resp = self._run_hindsight_operation(
            lambda client: _arecall_with_fail_open(
                client, plan.kwargs, auto_routed=plan.auto_routed,
                fail_open=plan.fail_open, fallback_kwargs=plan.fallback_kwargs,
                unfiltered_kwargs=plan.unfiltered_kwargs, exclude_tags=plan.exclude_tags,
            )
        )
        return _filter_routing_results(getattr(resp, "results", None), plan.exclude_tags, plan.max_results)
    ...
    def _reflect(self, query: str) -> str | None:
        ...
        if "types" in plan.kwargs:
            reflect_kwargs["fact_types"] = list(plan.kwargs["types"])
    ...
    def _do_recall(self, query: str) -> _RecallResult:
        ...
        try:
            if self._prefetch_method == "reflect":
                ...
                return _RecallResult(self._reflect(query) or "", 0)
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        if self._recall_sync:
            recall = _RecallResult("", 0) if self._recall_disabled() else self._do_recall(query)
            return self._finish_prefetch(recall.text, recall.count)
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        def _run():
            ...
            recall = self._do_recall(query)
            if recall.text:
                with self._prefetch_lock:
                    self._prefetch_result, self._prefetch_count = recall.text, recall.count
    ...
    def _build_retain_kwargs(self, content: str, *, context: str | None = None,
        ...
        route = self._select_recall_route(content) if self._recall_auto_route else None
        route_tags = _normalize_retain_tags((route or {}).get("retain_tags"))
        merged_tags = _normalize_retain_tags(list(self._retain_tags) + route_tags + _normalize_retain_tags(tags))
