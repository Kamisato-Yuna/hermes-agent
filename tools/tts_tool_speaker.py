    ...
    *tts_done_event* is **set** in ``finally`` so continuous voice mode knows playback finished."""
    ...
    try:
        ...
        if spoken_rewrite_enabled(spoken_cfg):
            logger.info("Streaming TTS disabled: semantic spoken rewrite waits for full reply")
            return
        ...
        stream_max_len = 0
        if streamer is None:
            sync_pipeline_cls = getattr(origin, "_SyncSentencePipeline", _SyncSentencePipeline)
            sync_pipeline = sync_pipeline_cls(stop_event)
        else:
            with contextlib.suppress(Exception):
