...
def _text_to_speech_single(
    text: str, file_str: Optional[str] = None, *, output_path: Optional[str] = None,
    provider: str, tts_config: Dict[str, Any],
    ...
    file_str = file_str or output_path
    if not file_str:
        return _error_json("TTS output path is required")
    try:
        if command_provider_config is not None:
...
def _synthesize_chunks(chunks: List[str], base_path: Path, generated_artifacts: set, **single_kwargs) -> tuple:
    ...
    for index, chunk in enumerate(chunks, start=1):
        ...
        raw_result = _text_to_speech_single(text=chunk, output_path=str(chunk_path), **single_kwargs)
        try:
...
def text_to_speech_tool(
    ...
    try:
        from tools.tts_spoken_script import (
            ...
            spoken_rewrite_enabled,
        )
        if spoken_rewrite_enabled(spoken_cfg):
            result = prepare_spoken_script(text, config=spoken_cfg)
            text = result.spoken_text
            logger.info(
                "Spoken rewrite: status=%s rewrite_used=%s fallback_used=%s input_chars=%d output_chars=%d request_id=%s",
                result.status, result.rewrite_used, result.fallback_used,
                original_input_chars, len(text), result.request_id,
            )
            if result.status == "fallback_notice":
                text = SPOKEN_FAILURE_NOTICE
        else:
            from tools.tts_text_normalize import prepare_spoken_text
            text = prepare_spoken_text(text, max_chars=None)
    except Exception:
        if spoken_cfg and spoken_cfg.get("enabled"):
            logger.exception("Spoken rewrite failed closed")
            return tool_error("Spoken rewrite failed; refusing to play the raw reply", success=False)
        text = text.strip()
    if not text:
...
_PLUGIN_COMPAT_LAZY = {
    ...
    '_SyncSentencePipeline': ('tools.tts_tool_speaker', '_SyncSentencePipeline'),
