...
class CLIChatTurnMixin:
    ...
    def _chat_setup_turn_audio(self, turn, message, voice_input):
        ...
        from cli import _ACCENT, _RST, _STREAM_PAD, _cprint, datetime, logger
        ...
        if self._voice_tts:
            try:
                from tools.tts_tool import _import_sounddevice, _load_tts_config, check_tts_requirements
                from tools.tts_tool_speaker import stream_tts_to_speaker
                from tools.tts_spoken_script import spoken_rewrite_enabled
                tts_config = _load_tts_config()
                if spoken_rewrite_enabled(tts_config.get("spoken_rewrite") if isinstance(tts_config, dict) else None):
                    logger.info("CLI streaming TTS disabled: semantic spoken rewrite waits for full reply")
                else:
                    _import_sounddevice()
