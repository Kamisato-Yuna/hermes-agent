...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("start", "确认平台启动信号,无需回复", "Session",
               gateway_only=True, busy_policy="dispatch", busy_handler="start"),
    CommandDef("new", "开启新会话(全新会话 ID 与历史)", "Session",
               aliases=("reset",), args_hint="[name]",
               busy_policy="interrupt_then_dispatch", busy_handler="new"),
    CommandDef("topic", "启用或查看 Telegram 私聊主题会话", "Session",
    ...
    CommandDef("history", "Show conversation history", "Session",
               cli_only=True, desktop="terminal"),
    CommandDef("save", "导出当前会话(单独 /save 显示用法)", "Session",
               args_hint="<json|md|html> [filename] [redact]"),
    CommandDef("retry", "重试上一条消息(重新发送给助手)", "Session"),
    ...
    CommandDef("compress", "压缩会话上下文(加 here [N] 保留最近 N 轮;--preview 预览效果)", "Session",
               aliases=("compact",), args_hint="[here [N] | focus topic | --preview|--dry-run]"),
    CommandDef("rollback", "列出或恢复文件系统检查点(恢复会保留你的手动编辑;--all 覆盖)", "Session",
    ...
    CommandDef("pause", "全局暂停新的工作(紧急停止);'/pause off' 恢复", "Session",
               gateway_only=True, args_hint="[reason | off]",
               busy_policy="dispatch"),
    CommandDef("approve", "批准一个待处理的高风险命令", "Session",
               gateway_only=True, args_hint="[session|always]", busy_policy="dispatch",
               desktop="messaging"),
    CommandDef("deny", "拒绝一个待处理的高风险命令(可附原因)", "Session",
               gateway_only=True, args_hint="[all] [reason]", busy_policy="dispatch",
               desktop="messaging"),
    CommandDef("bg", "在独立后台会话运行提示", "Session",
               args_hint="<prompt>", busy_policy="dispatch"),
    CommandDef("btw", "在不打断当前对话的前提下提一个附属问题", "Session",
               args_hint="<question>", busy_policy="dispatch"),
    CommandDef("agents", "查看活动的助手与运行中的任务", "Session",
    ...
    CommandDef("queue", "将提示排入下一轮(不打断)", "Session",
               aliases=("q",), args_hint="<prompt>",
               busy_policy="dispatch", busy_handler="queue"),
    CommandDef("steer", "在下一个工具调用后注入一条消息,不打断", "Session",
               args_hint="<prompt>", busy_policy="dispatch", busy_handler="steer"),
    CommandDef("goal", "设定一个跨轮次推进的标准目标,直至达成", "Session",
    ...
    CommandDef("heartbeat", "设定一个闲置时重新进入本会话的循环提示", "Session",
               aliases=("hb",), args_hint="[every <interval> <prompt> | status | pause | resume | clear]",
               subcommands=("status", "pause", "resume", "clear"),
               busy_policy="dispatch"),
    CommandDef("refine", "现在回顾本次对话,并把经验存入 memory/skills", "Session",
               args_hint="[focus instructions]"),
    CommandDef("review", "派生独立子助手回顾刚讨论的工作(PR、代码、文档)", "Session",
               args_hint="[review instructions]"),
    CommandDef("loop", "在本会话按固定间隔循环重跑某个提示", "Session",
               aliases=("proactive",),
    ...
    CommandDef("plan", "把 markdown 实施计划写入 .hermes/plans/,不做任何执行", "Session",
               args_hint="[task]"),
    CommandDef("moa", "用默认 Mixture of Agents 预设跑一次提示,然后恢复原模型", "Session",
               args_hint="<prompt>", busy_policy="reject", busy_handler="moa"),
    CommandDef("subgoal", "为当前目标增加或管理额外标准", "Session",
               args_hint="[text | remove N | clear]", busy_policy="dispatch"),
    CommandDef("status", "显示会话、模型、Token 与上下文信息", "Session",
               busy_policy="dispatch"),
    CommandDef("egress", "显示 Docker 出口代理状态", "Session",
               args_hint="[status]", subcommands=("status",),
               busy_policy="dispatch", busy_handler="egress",
               execute="egress"),
    CommandDef("context", "显示详细上下文窗口视图(用量、分类、压缩统计与吞吐)", "Session",
               aliases=("ctx",), args_hint="[all]", subcommands=("all",),
               busy_policy="dispatch"),
    CommandDef("whoami", "显示你的斜杠命令访问权限(管理员 / 用户)", "Info"),
    CommandDef("profile", "显示当前激活的 profile 名称与主目录", "Info",
               busy_policy="dispatch", execute="profile"),
    CommandDef("sethome", "将本会话设为 home 频道", "Session",
               gateway_only=True, aliases=("set-home",), desktop="terminal"),
    CommandDef("resume", "恢复一个已被命名的会话", "Session",
    ...
    CommandDef("config", "Show current configuration", "Configuration",
               cli_only=True, desktop="terminal"),
    CommandDef("model", "切换模型(会话级;--global 持久化)", "Configuration",
               args_hint="[model] [--provider name] [--global|--session] [--refresh]",
               busy_policy="reject", busy_handler="model", desktop="hidden"),
    CommandDef("codex-runtime", "切换 OpenAI/Codex 模型的 codex app-server 运行时",
               "Configuration", aliases=("codex_runtime",),
               args_hint="[auto|codex_app_server]",
               busy_policy="reject", busy_handler="codex-runtime"),

    CommandDef("personality", "设定预设人格", "Configuration",
    ...
    CommandDef("diff", "显示工作目录的 git 改动", "Info",
    ...
    CommandDef("wake", "Toggle the 'Hey Hermes' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "控制 Hermes 繁忙时消息的行为", "Configuration",
    ...
    CommandDef("reload", "Reload .env variables into the running session", "Tools & Skills",
               cli_only=True, desktop="terminal"),
    CommandDef("reload-mcp", "从配置重载 MCP 服务器", "Tools & Skills",
               aliases=("reload_mcp",), desktop="advanced"),
    CommandDef("reload-skills", "重新扫描 ~/.hermes/skills/ 以发现新装或移除的技能",
    ...
    # Info
    CommandDef("commands", "浏览所有命令与技能(分页)", "Info",
               gateway_only=True, args_hint="[page]", busy_policy="dispatch",
               execute="gateway_commands"),
    CommandDef("help", "显示可用命令(/help skills 列出技能命令,/help <text> 过滤)", "Info", busy_policy="dispatch",
    ...
    CommandDef("restart", "排空活动运行后优雅重启网关", "Session",
               gateway_only=True, busy_policy="dispatch", desktop="terminal"),
    CommandDef("usage", "显示 Token 用量与速率限制;`reset` 赎回已存入的 Codex 限额重设", "Info",
    ...
    CommandDef("topup", "显示你的 Nous 余额并在门户管理计费", "Info"),
    CommandDef("insights", "显示用量洞察与分析", "Info",
               args_hint="[days]", desktop="advanced"),
    CommandDef("platforms", "Show gateway/messaging platform status", "Info",
               cli_only=True, aliases=("gateway",), desktop="terminal"),
    CommandDef("platform", "暂停、恢复或列出故障的网关平台", "Info",
    ...
    CommandDef("update", "升级 Hermes Agent 到最新版本", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "显示 Hermes Agent 版本", "Info", aliases=("v",),
