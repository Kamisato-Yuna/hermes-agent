...
def _speak_streaming(text: str, stop_event: Optional[threading.Event]) -> bool:
    ...
    if spoken_rewrite_enabled(tts_config.get("spoken_rewrite") if isinstance(tts_config, dict) else None):
        return False
    if resolve_streaming_provider(tts_config) is None:
        return False
...
def _speak_text_streaming(text: str, stop_event: Optional[threading.Event]) -> bool:
    """Compatibility name retained for plugins and older voice-wrapper tests."""
    return _speak_streaming(text, stop_event)
